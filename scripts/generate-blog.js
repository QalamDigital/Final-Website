...
function localPostLink(prefix, post) {
  return `${prefix}blog/${post.slug}/index.html`;
}
...
function nav(prefix, active = 'blog') {
  const links = [
    ...
    ['Blog', `${prefix}blog/blog_index.html`, 'blog'],
    ...
    <div class="footer-grid">
      ...
      <div class="footer-col">
        ...
        <ul>
          ...
          <li><a href="${prefix}blog/blog_index.html">Blog</a></li>
...
function blogIndexSchema(posts, pageUrl, title, description) {
  return {
    ...
    '@graph': [
      ...
      breadcrumb([['Home', `${SITE_URL}/`], ['Blog', `${SITE_URL}/blog/blog_index.html`]], `${pageUrl}#breadcrumb`)
    ]
  };
}
...
function postSchema(post, prev, next) {
  return {
    ...
    '@graph': [
      ...
      breadcrumb([['Home', `${SITE_URL}/`], ['Blog', `${SITE_URL}/blog/blog_index.html`], [post.title, postUrl(post)]], `${postUrl(post)}#breadcrumb`),
      {
        '@type': 'FAQPage',
        '@id': `${postUrl(post)}#faq`,
...
function renderBlogIndex(posts, page, totalPages) {
  const prefix = page === 1 ? '../' : '../../../';
  const pageUrl = page === 1 ? `${SITE_URL}/blog/blog_index.html` : `${SITE_URL}/blog/page/${page}/blog_index.html`;
...
function renderPagination(page, totalPages) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    const href = page === 1
      ? (i === 1 ? './blog_index.html' : `page/${i}/blog_index.html`)
      : (i === 1 ? '../../blog_index.html' : `../../page/${i}/blog_index.html`);
...
<section id="featured-blog" class="featured-blog-section">
  <div class="container">
    <div class="portfolio-header reveal">
      ...
      <a href="blog/blog_index.html" class="btn-ghost">View All Posts</a>
    </div>
    <div class="featured-blog-grid">
      ${featured.map(post => `<article class="featured-blog-card reveal">
        <a class="featured-blog-image" href="blog/${post.slug}/index.html"><img src="${post.image}" alt="${escapeHtml(post.imageAlt)}"></a>
        <div class="featured-blog-body">
          <span>${escapeHtml(post.category)} &middot; ${post.readMinutes} min read</span>
          <h3><a href="blog/${post.slug}/index.html">${escapeHtml(post.title)}</a></h3>
          <p>${escapeHtml(post.excerpt)}</p>
          <a class="blog-read-more" href="blog/${post.slug}/index.html">Read More</a>
        </div>
      </article>`).join('\n')}
    </div>
  </div>
...
function updateRootNavigation() {
  for (const file of ['index.html', 'about.html', 'portfolio.html', 'contact.html']) {
    ...
    // Replace any reference to blog/index.html or blog/ with blog/blog_index.html
    html = html.replace(/href="blog\/(?:index\.html)?"/g, 'href="blog/blog_index.html"');
...
function updateSitemap(posts, totalPages) {
  const urls = [
    ...
    ['https://www.qalamdigital.com/blog/blog_index.html', TODAY, 'weekly', '0.9']
  ];
  for (let page = 2; page <= totalPages; page++) {
    urls.push([`https://www.qalamdigital.com/blog/page/${page}/blog_index.html`, TODAY, 'weekly', '0.7']);
  }
...
function generateBlog() {
  ...
  for (let page = 1; page <= totalPages; page++) {
    const filePath = page === 1
      ? path.join(ROOT, 'blog', 'blog_index.html')
      : path.join(ROOT, 'blog', 'page', String(page), 'blog_index.html');
